...
class Server:
    ...
    def start_server(self, conn=5, buffer=4096, host="", port=8080):
        try:
            self.write_log(self.getTimeStampp() + "   \n\nStarting Server\n\n")

            self.listen(conn, buffer, host, port)
    ...
    def listen(self, No_of_conn, buffer, host, port):
        try:
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            s.bind((host, port))
            s.listen(No_of_conn)
            print(self.getTimeStampp() + "   Listening...", "{}:{}".format(host,port))
            self.write_log(
...
if __name__ == "__main__":
    ...
    server = Server(blacklisted_ips=['127.0.0.81'],blacklist_websites=['facebook'])
    server.start_server(host="127.0.0.1",port=9090)